...
#set directory to your scGWAS results folder 
setwd("")
local.path = ""
...
#prefix of scGWAS output files from Java package
input_header = ""
#prefix of output plots to save files 
out_header = ""

#Name trait and panel 
trait=""
panel=""

#import table with GWAS gene scores
ff = paste(input_header,"..shared_genes.scores.calibration.txt", sep="")
all.weight.data = read.csv(ff, as.is=T, header=T, sep="\t")

#import file with non-random modules created 
ff = paste(input_header,".combined-z.txt", sep="")
dat = read.csv(ff, header=T, as.is=T, sep="\t")
dat = dat[!is.na(dat[,8]),]
...
#import file containing random module definitions 
fr = paste(input_header, ".all.random_modules.txt", sep="")
datr = read.csv(fr, header=F, as.is=T, sep="\t")
...
for(cell in cells){
	...
	if(length(genes) > 5){
		...
		res2 <- fgsea(pathways = list(A=genes), stats = gwas.data, minSize=5,maxSize=1000, nproc=1, scoreType="pos") #removed nperm=10000
		
		scrn.data = weight.data[,9]; names(scrn.data) = weight.data[,1]
		res4 <- fgsea(pathways = list(A=genes), stats = scrn.data, minSize=5,maxSize=1000, nproc=1, scoreType="pos")  # removed nperm=10000
